...
class SoftEncoderRationaleModel(RationaleBaseModel):
    ...
    def _generate_attention(self, **kwargs) -> Dict[str, Any]:
        ...
        tokens = [m['tokens'] for m in kwargs['metadata']]
        attentions = scorer_dict['attentions']
        assert attentions.shape[0] == 5*len(tokens), breakpoint()
    ...
    def _decode(self, output_dict):
        ...
        new_output_dict["comprehensiveness_classification_scores"] = [
            dict(zip(self._output_labels, list(x))) for x in output_dict["comprehensiveness_classification_scores"]
        ]
        new_output_dict["sufficiency_classification_scores"] = [
            dict(zip(self._output_labels, list(x))) for x in output_dict["sufficiency_classification_scores"]
        ]
    ...
    def regenerate_tokens(self, attentions, metadata, threshold, labels):
        ...
        for b in range(attentions_cpu.shape[0]):
            ...
            max_length = math.ceil(len(sentence) * threshold)

            top_ind = np.argsort(attn)[-max_length:]
            new_tokens.append([x for i, x in enumerate(sentence) if i in top_ind])
